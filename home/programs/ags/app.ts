...
import NetworkMenuWindow from "./widget/networkMenu/NetworkMenuWindow";
import BluetoothMenuWindow from "./widget/bluetoothMenu/BluetoothMenuWindow";
import MediaPlayerMenuWindow from "./widget/mediaPlayerMenu/MediaPlayerMenuWindow";
  ...
  main(...args: Array<string>) {
    ...
    hyprland.monitors.forEach(monitor => {
      if (args.includes("sidebar")) {
        SideBar(monitor.id)
        Calendar(monitor.id, Astal.WindowAnchor.BOTTOM | Astal.WindowAnchor.LEFT)
      } else if (args.includes("topbar")) {
        TopBar(monitor.id)
        Calendar(monitor.id, Astal.WindowAnchor.TOP)
      } else if (ratio > 2) {
        SideBar(monitor.id)
        Calendar(monitor.id, Astal.WindowAnchor.BOTTOM | Astal.WindowAnchor.LEFT)
      } else {
      ...
      VolumeAlert(monitor.id)
      BrightnessAlert(monitor.id)
    })
    SystemMenuWindow()
    NetworkMenuWindow()
    BluetoothMenuWindow()
    MediaPlayerMenuWindow()
    ChargingAlertSound()
