...
}:
{
  ...
  programs.starship = {
    enable = true;
    # starship's `init` bakes the first `starship` on PATH into its prompt hooks,
    # which is the mutable ~/.local/state/nix/profile/bin copy. A switch rebuilds
    # that profile and dangles the symlink, breaking the prompt. initContent below
    # re-adds the init with the immutable store path resolving first.
  ...
  programs.zsh = {
    ...
    initContent = lib.mkMerge [
      ...
      # starship init, run with the immutable store bin resolving first so the
      # prompt hooks bake the /nix/store path, which never dangles. See
      # programs.starship.enableZshIntegration above.
      (lib.mkOrder 1000 ''
        if [[ $TERM != "dumb" ]]; then
